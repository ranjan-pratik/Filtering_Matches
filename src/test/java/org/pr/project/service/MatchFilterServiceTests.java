...
public class MatchFilterServiceTests {
	...
	private MatchFilterServiceImpl filterService;
	...
	public void init() throws JsonMappingException, JsonProcessingException {
		filterService = new MatchFilterServiceImpl();
	...
	public void test_getDefaultList() {
		...
		Mockito.when(matchFilterRepository.findAll())
				.thenReturn(unfilteredVO.getMatches());
	...
	public void test_getAll_emptyDataStore() {
		...
		Mockito.when(matchFilterRepository.findAll())
				.thenReturn(new ArrayList<Match>());
	...
	public void test_getFiltered_noFilterApplied_nullPassed_shouldReturnAll() {
		...
		assertThat(unfilteredList).isNotNull();
		assertThat(unfilteredList.size())
				.isEqualTo(unfilteredVO.getMatches().size());
	}
	...
	public void test_getFiltered_noFilterApplied_emptyListPassed_shouldReturnAll() {
		...
		List<Match> unfilteredList = filterService
				.getFilteredMatches(new ArrayList<AbstractFilter>());
	...
	public void test_getFiltered_ageFilterApplied_shouldReturnFiltered() {
		...
		Match candidate1 = new Match("Candidate1", "some Job", 23,
				Religion.Christian, "", 163d, 15d, 5, true,
		...
		Mockito.when(matchFilterRepository.findByCustomCriteria(Mockito.any()))
				.thenReturn(filteredData);

		List<Match> filteredList = filterService
				.getFilteredMatches(filtersToApply);
